...
CREATE OR REPLACE TABLE `nbcu-ds-sandbox-a-001.SLi_sandbox.Push_EOM_Churn_Quarterly_Weighted_{report}` AS (
    ...
    -- Required to full join with quarterly cohort - include users who are kicked from measurement audience due to email unsubs and email received for holdout.
    -- This is approximately 1% of the cohort - significant enough to affect metrics.
    ...
    FROM `nbcu-ds-sandbox-a-001.SLi_sandbox.SILVER_PUSH_CHANNEL_PERFORMANCE_MONTHLY` base
    JOIN month_ends
...
CREATE OR REPLACE TABLE `nbcu-ds-sandbox-a-001.SLi_sandbox.Push_Channel_Base_{report}` AS (
    ...
    LEFT JOIN `nbcu-ds-sandbox-a-001.SLi_sandbox.Push_EOM_Churn_Quarterly_Weighted_{report}` w
    ON a.aid = w.aid
